...
export async function createPayable(name: string, amount: number, category: string, dueDay: number): Promise<void> {
  const db = await getDb();
  await db.runAsync(
    `
      INSERT INTO payables (name, amount, category, due_day, is_paid, paid_at, paid_account_name, created_at)
      VALUES (?, ?, ?, ?, 0, '', '', ?)
    `,
    name.trim(),
...
export async function listPayables(): Promise<Payable[]> {
  ...
  const rows = await db.getAllAsync<PayableRow>(
    `
      SELECT id, name, amount, category, due_day, is_paid, paid_at, paid_account_name, created_at
      FROM payables
      ORDER BY
        CASE
